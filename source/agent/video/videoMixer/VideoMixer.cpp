    ...
    , m_maxInputCount(16)
{
    ...
    ELOG_INFO("Init maxInput(%u), rootSize(%u, %u), bgColor(%u, %u, %u), fitPolicy(%d)", m_maxInputCount, rootSize.width, rootSize.height, bgColor.y, bgColor.cb, bgColor.cr, (int)config.fitPolicy);

    m_frameMixer.reset(new VideoFrameMixerImpl(m_maxInputCount, rootSize, bgColor, true, (int)config.fitPolicy));
}
